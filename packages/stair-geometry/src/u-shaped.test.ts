...
describe('calculateUShaped', () => {
  ...
  test('handles minimum floor height boundary', () => {
    ...
    const result = calculateUShaped(input);
    expect(result.totalSteps).toBeGreaterThanOrEqual(3);
    expect(result.steps.filter(s => s.isWinder).length).toBe(6);
  });

  test('middle march has lateral offset', () => {
    const input: StairInput = {
      type: 'u-shaped',
      floorHeight: 2700,
      openingWidth: 1500,
    ...
    const result = calculateUShaped(input);
    const winderSteps = result.steps.filter(s => s.isWinder);
    const firstWinderEndIndex = result.steps.indexOf(winderSteps[winderSteps.length - 1]);
    
    const middleMarchStart = firstWinderEndIndex + 1;
    const middleMarchStep = result.steps[middleMarchStart];
    expect(middleMarchStep.x).toBeGreaterThan(0);
  });

  test('first and last march have x = 0', () => {
    const input: StairInput = {
      type: 'u-shaped',
      floorHeight: 2700,
      openingWidth: 1500,
    ...
    expect(result.steps[0].x).toBe(0);
    const lastStep = result.steps[result.steps.length - 1];
    expect(lastStep.x).toBe(0);
  });

  test('winder steps have varying rotationY', () => {
    const input: StairInput = {
      type: 'u-shaped',
      floorHeight: 2700,
      openingWidth: 1500,
    ...
    const firstTurnWindners = winderSteps.slice(0, 3);
    const secondTurnWindners = winderSteps.slice(3, 6);
    
    firstTurnWindners.forEach((step, i) => {
      if (i > 0) {
        expect(step.rotationY).not.toBe(firstTurnWindners[i - 1].rotationY);
      }
    });
    
    secondTurnWindners.forEach((step, i) => {
      if (i > 0) {
        expect(step.rotationY).not.toBe(secondTurnWindners[i - 1].rotationY);
      }
    });
  });
});