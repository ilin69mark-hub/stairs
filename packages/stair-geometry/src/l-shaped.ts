...
export function calculateLShaped(input: StairInput): StairGeometry {
  ...
  const turnRadius = Math.max(tread * SNIP.WINDER_RADIUS_FACTOR, input.openingLength * 0.3);

  for (let i = 0; i < winderCount; i++) {
    const t = (i + 1) / (winderCount + 1);
    const angle = t * (Math.PI / 2);
    const radius = tread * (SNIP.WINDER_RADIUS_MIN_FACTOR + t * (SNIP.WINDER_RADIUS_FACTOR + SNIP.WINDER_RADIUS_MIN_FACTOR));
