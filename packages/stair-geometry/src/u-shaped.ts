...
import { calculateStepsCount, calculateTreadDepth, validateSnip, validateWinderSteps, validateStairWidth } from './utils';
...
export function calculateUShaped(input: StairInput): StairGeometry {
  ...
  const turnRadius = Math.max(tread * SNIP.WINDER_RADIUS_FACTOR, input.openingLength * 0.25);
  ...
  const lateralOffset = turnRadius * SNIP.U_SHAPED_TURN_OFFSET_FACTOR;
  ...
  for (let i = 0; i < winderPerTurn; i++) {
    ...
    const stepIndex = segmentSteps * 2 + winderPerTurn + i;
    const yPos = (stepIndex + 1) * rise;
  ...
  for (let i = 0; i < upperSteps; i++) {
    const stepIndex = segmentSteps * 2 + winderPerTurn * 2 + i;
    const yPos = (stepIndex + 1) * rise;
  ...
  const snipValidation = validateSnip(rise, tread, Math.atan(rise / tread) * (180 / Math.PI), steps.length);
  warnings.push(...snipValidation.warnings);

  const winderWarnings = validateWinderSteps(steps);
  warnings.push(...winderWarnings);
